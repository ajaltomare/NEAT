...
def do_sim(chrom, reference, ref_index, n_handling, input_variants, paired_end, fragment_size, fraglen_distribution, read_len, 
    only_vcf, fasta_instead, debug, gc_window_size, gc_scale_val, input_bed, input_regions, off_target_scalar, off_target_discard,
    low_cov_thresh, ploids, mut_model, mut_rate, cancer, cancer_model, force_coverage, coverage, se_class, discard_bed, 
    discard_regions, out_prefix_name, n_max_qual, unmapped_records, indices_by_ref_name, no_fastq, output_file_writer, save_bam, save_vcf,
    read_name_count
):
    output_file_writer.open(f'thread_{current_process().name}')
...
def main(raw_args=None):
    ...
    per_process_args = [(ri, reference, ref_index, n_handling, input_variants, paired_end, fragment_size, fraglen_distribution, read_len, 
        only_vcf, fasta_instead, debug, gc_window_size, gc_scale_val, input_bed, input_regions, off_target_scalar, off_target_discard,
        low_cov_thresh, ploids, mut_model, mut_rate, cancer, cancer_model, force_coverage, coverage, se_class, discard_bed, discard_regions,
        out_prefix_name, n_max_qual, unmapped_records, indices_by_ref_name, no_fastq, output_file_writer, save_bam, save_vcf, read_name_count) 
            for ri in range(len(ref_index))]

    with Pool(num_processors) as pool:
        pool.starmap(do_sim, per_process_args)

    agg_r1_fq = f'{out_prefix}_R1.fastq.gz'
    r1_fqs = [p.as_posix() for p in pathlib.Path(out_prefix).parent.glob("*_read1.fq.gz")]

    agg_r2_fq = f'{out_prefix}_R2.fastq.gz'
    r2_fqs = [p.as_posix() for p in pathlib.Path(out_prefix).parent.glob("*_read2.fq.gz")]

    r1_cmd = ['cat',] + r1_fqs
    with open(agg_r1_fq, 'wb') as fq_handle:
        proc = run(r1_cmd, stdout=fq_handle)

    r2_cmd = ['cat',] + r2_fqs
    with open(agg_r2_fq, 'wb') as fq_handle:
        proc = run(r2_cmd, stdout=fq_handle)
